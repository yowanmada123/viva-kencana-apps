...
class StockOpnameDtl {
  ...
  /// =======================
  factory StockOpnameDtl.fromMap(Map<String, dynamic> map) {
    return StockOpnameDtl(
      trId: map['tr_id'] ?? '',
      millId: map['mill_id'] ?? '',
      ...
      addId: map['add_id'] ?? '',
      torId: map['tor_id'] ?? '',
      panjang: double.tryParse(map['panjang'].toString()) ?? 0,
      batchId: map['batch_id'] ?? '',
      whId: map['wh_id'] ?? '',
      binId: map['bin_id'] ?? '',
      qtyAwal: double.tryParse(map['qty_awal'].toString()) ?? 0,
      qtyOpname: double.tryParse(map['qty_opname'].toString()) ?? 0,
      remark: map['remark'] ?? '',
      dtCek: map['dt_cek'] != null ? DateTime.tryParse(map['dt_cek']) : null,
      dtModified:
          map['dt_modified'] != null
              ? DateTime.tryParse(map['dt_modified'])
              : null,
      userId: map['user_id'] ?? '',
      coilId: map['coil_id'] ?? '',
    );
  ...
  factory StockOpnameDtl.fromJson(String source) =>
      StockOpnameDtl.fromMap(json.decode(source));

  @override
  String toString() {
    return 'StockOpnameDtl(tr_id: $trId, prod_code: $prodCode, NamaBarang: $namaBarang, qty_opname: $qtyOpname)';
  }
  ...
  bool operator ==(Object other) {
    ...
    return other is StockOpnameDtl &&
        other.trId == trId &&
        other.millId == millId &&
